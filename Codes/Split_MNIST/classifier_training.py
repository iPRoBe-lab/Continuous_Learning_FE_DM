...
def ViTFeatureExtractor():
    # ViT-based Experiments
    args = get_args(sys.argv[1:])
    Train_Task = '1'
    Tasks_array = [['Task_1', 'Task_2', 'Task_3', 'Task_4', 'Task_5']]
    ...
    # Feature Extraction
    # weight_path = 'D:/Source_Code/Mean-Shifted-Anomaly-Detection-main/TempData/Results/ViT/MNIST/Class_0_1/ViT_best.pth'
    # train_loader = torch.utils.data.DataLoader(train_dataset_splits[Train_Task],
    #                                            batch_size=args.batch_size, shuffle=True, num_workers=args.workers)
    # weight_path= '../TempData/data/MNIST/Task_5/IntraClassLoss/ViT_best.pth'
    # save_folder = os.path.join(args.dataroot, args.dataset,'Task_5/IntraClassLoss')
    ...
    #                                          num_workers=1)
    # agent.extractViTFeatures(val_loader,weight_path, save_folder)
    ...
    # Center Calculation
    # Tasks = ['Task_1/IntraClassLoss', 'Task_2/IntraClassLoss', 'Task_3/IntraClassLoss', 'Task_4/IntraClassLoss', 'Task_5/IntraClassLoss']
    # save_folder = os.path.join(args.dataroot, args.dataset)
    # agent.computeCenters(save_folder,Tasks)

    # Evaluation
    Val_Task = 'All' # ['1','2','3','4','5', 'All']
    save_folder = os.path.join(args.dataroot, args.dataset, 'Task_' + Val_Task)
    distType= 'LocalOutlierFactor'
    if Val_Task == 'All':
        val_dataset_all = torch.utils.data.ConcatDataset(val_dataset_splits.values())
        val_loader = torch.utils.data.DataLoader(val_dataset_all,
                                                     batch_size=1, shuffle=False,
        ...
        val_loader = torch.utils.data.DataLoader(val_dataset_splits[Val_Task],
                                                 batch_size=1, shuffle=False,
                                                 num_workers=1)
    for Trained_Tasks in Tasks_array:
        agent.evaluateViT(val_loader, save_folder, Trained_Tasks,distType)

def run(args,repeat_no):
    ...
    if args.offline_training:  # Non-incremental learning / offline_training / measure the upper-bound performance
        ...
        for i in range(len(task_names)):
            ...
            save_folder = os.path.join(args.dataroot, args.dataset, 'Task_' + train_name)
            ...
            # save model
            agent.save_model(os.path.join(save_folder,'Task_'+train_name+'_'+str(repeat_no)))
...
if __name__ == '__main__':
    ...
    for reg_coef in reg_coef_list:
        ...
        for r in range(args.repeat):
            ...
            # Run the experiment
            acc_table, task_names = run(args,r)
            print(acc_table)
    ...
    # Extract ViT Features for each subtask
    #ViTFeatureExtractor()